...
export function calculateGeodesic(map, geometry, units = 'meters') {
  ...
  if (type === 'Polygon') {
    ...
    return (Math.PI * geometry.getRadius() ** 2) * CONVERSION[units] // this should be able to use `getArea` but needs to be tested first
  }
  return 0
}
...
export function getMeasurementText(opts, geom) {
  const {
    distanceUOM, areaUOM, map, translations, language,
  } = opts
  const sourceProj = map.getView().getProjection()

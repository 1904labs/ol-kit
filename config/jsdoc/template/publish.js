...
function buildNav(members) {
  ...
  const types = ['tutorials', 'modules', 'externals', 'namespaces', 'classes',
...
exports.publish = function (taffyData, opts, tutorials) {
  ...
  staticFiles.forEach((fileName) => {
    const toDir = fs.toDir(fileName.replace(staticDir, path.join(outdir, 'static')))
  ...
  staticFiles.forEach((fileName) => {
    const toDir = fs.toDir(fileName.replace(styleDir, path.join(outdir, 'styles')))
  ...
  scriptFiles.forEach((fileName) => {
    const toDir = fs.toDir(fileName.replace(scriptDir, path.join(outdir, 'scripts')))
  ...
  members.components.forEach((c) => {
    ...
    if (exists) {
      ...
      const parsed = markdown.getParser()(fileContents)
  ...
  const pagesSrc = path.normalize(env.opts.pages)

  fs.readdir(pagesSrc, (err, files) => {
    if (err) {
    ...
    files.forEach((file, index) => {
      const filePath = path.join(pagesSrc, file)
      const content = fs.readFileSync(filePath, env.opts.encoding)
      const parsed = markdown.getParser()(content)
      ...
      const pagePath = path.join(outdir, file.replace('.md', '.html'))
      let html = view.render('page.tmpl', pageData)
